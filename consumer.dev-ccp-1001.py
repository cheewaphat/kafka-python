...
def toFile(message):  
    print "write file "+ "/tmp/lab/json/"+message.topic+"_"+str(message.timestamp)+".json"    
    path = "/tmp/lab/json/"+message.topic+"_"+str(message.timestamp)+".json"    
    with open(path, "a") as f:
        f.write("%(value)s\n" % {"value":message.value})         
    # for prm in message_parameters:
    #      csvwriter.writerow(prm)
